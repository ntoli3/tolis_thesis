 close all; clear; clc;

% orismos diastasewn tou plegmatos
Nx = 5; %arithmos kombwn sth dieuthunsh x
Ny = 4; %arithmos kombwn sth dieuthunsh y

% orismos twn oriwn toy xwrou
x_min = 0;
x_max = 1;
y_min = 0;
y_max = 1;

% kyklos
x_c = 0.5;
y_c = 0.5;
radius = 0.24; 

[node_coords, element_nodes] = create_mesh(Nx, Ny, x_min, x_max, y_min, y_max);
[phi_all] = find_level_sets_circle(x_c, y_c, radius, node_coords);
%[elem_intersected] = check_circle_intersection(x_c, y_c, radius, node_coords, element_nodes);
[intersected_elements] = check_intersection_elements_lsm( ...
    phi_all, element_nodes);






