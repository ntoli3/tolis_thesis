classdef FemModel < handle
    ...
    methods
        function obj = FemModel() %Constructor
        ...
        function [ke] = buildElementStiffness(obj, element_id)
            ...
            node_ids = obj.element_nodes(element_id,:);
            for n = 1:4
                id = node_ids(n);
                coords = obj.node_coords(id, :);
                nodes_of_element(n, :) = coords;
            end

            if obj.intersected_elements(element_id) == 0
                % Material properties
                E_neg = obj.material_neg.E;
                v_neg = obj.material_neg.v;
                t_neg = obj.material_neg.thickness;
                E_pos = obj.material_pos.E;
                v_pos = obj.material_pos.v;
                t_pos = obj.material_pos.thickness;
                
                % Level sets of nodes of element
                nodal_level_sets = zeros(4, 1);
                for n = 1:4
                    id = node_ids(n);
                    nodal_level_sets(n, 1) = obj.phi_nodes_all(id, 1);
                end

                ke = xquad4_stiffness(nodes, E_neg, E_pos, v_neg, v_pos, ...
                    t_neg, t_pos, nodal_level_sets, obj.psi_handle);

            elseif obj.intersected_elements(element_id) == 1
                E = obj.material_pos.E;
                v = obj.material_pos.v;
                t = obj.material_pos.thickness;
                ke = quad4_stiffness(nodes_of_element, E, v, t);
            else
                E = obj.material_neg.E;
                v = obj.material_neg.v;
                t = obj.material_neg.thickness;
                ke = quad4_stiffness(nodes_of_element, E, v, t);
            end
        end
